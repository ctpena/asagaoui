+++
title = 'Checkbox'
summary = 'Checkbox'
+++

A checkbox is a form element that allows users to select one or more items from multiple options. AsagaoUI provides an implementation that emphasizes accessibility and usability.

```html {preview="true"}
<label class="checkbox">
    <input class="checkbox-input" type="checkbox">
    Label
</label>
```

## Size

You can change the size of the checkbox by adding `is-sm`, `is-md`, or `is-lg` classes.

- `sm` Default size
- `md`
- `lg` 

```html {preview="true"}
<label class="checkbox is-lg">
    <input class="checkbox-input" type="checkbox">
    lg size
</label>
<label class="checkbox is-md">
    <input class="checkbox-input" type="checkbox">
    md size
</label>
<label class="checkbox is-sm">
    <input class="checkbox-input" type="checkbox">
    sm size
</label>
```

## Example

Using `form-group` makes error display concise.

```html {preview="true"}
<div class="form-group">
    <label for="test" class="form-label">
        Tokyo 23 Wards
        <span class="form-necessity">*Required</span>
    </label>
    <div class="form-support">Please select all applicable wards.</div>
    <div>
        <label class="checkbox">
            <input class="checkbox-input" type="checkbox">
            Adachi-ku
        </label>
        <label class="checkbox">
            <input class="checkbox-input" type="checkbox">
            Arakawa-ku
        </label>
        <label class="checkbox">
            <input class="checkbox-input" type="checkbox">
            Itabashi-ku
        </label>
        <label class="checkbox">
            <input class="checkbox-input" type="checkbox">
            Edogawa-ku
        </label>
    </div>
</div>
```
